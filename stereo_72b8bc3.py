...
NET = 'leastereo'
CRF = True
CROPPED_H, CROPPED_W = (288, 624)  # (264, 768)  # (256, 768)  # (288, 640)  # (304, 640)  # (240, 624) # (320, 1216/1040+16)
MIRROR = False
...
def get_dataset(shuffle=True, name='*', scene='*'):
    ls_L = tf.data.Dataset.list_files(DATA_DIR + f"/{name}/{scene}/frames/rgb/Camera_0/*", False)
    ls_R = tf.data.Dataset.list_files(DATA_DIR + f"/{name}/{scene}/frames/rgb/Camera_1/*", False)
    ls_TL = tf.data.Dataset.list_files(DATA_DIR + f"/{name}/{scene}/frames/depth/Camera_0/*", False)
    ls_TR = tf.data.Dataset.list_files(DATA_DIR + f"/{name}/{scene}/frames/depth/Camera_1/*", False) if MIRROR else None
    dst = tf.data.Dataset.zip((ls_L, ls_R, ls_TL, ls_TR) if MIRROR else (ls_L, ls_R, ls_TL)).cache()
    if shuffle:
...
if __name__ == "__main__":
    ...
    # log_dir = 'logs/e2e/' + time + f'-{DS_NAME}-{CROPPED_H}x{CROPPED_W}_d{DISPARITIES}_IR-{INFER_RATE}_ITER-{INFER_ITER}_TR-{LEARN_RT}_BS-{BATCH_SIZE}_{NET}-fromds'
    weights_dir = log_dir = 'logs/e2e/20210425-160209-kitti2012C-288x624_d192_IR-0.01_ITER-21_TR-0.001_BS-9_leastereo-fromds'
    ...
    #     else:
    #         return lr / 8
    # scheduler_cbk = tf.keras.callbacks.LearningRateScheduler(scheduler, verbose=1)
    ...
    with mirrored_strategy.scope():
        model = CrfRnn(max_disparity=DISPARITIES, infer_iter=INFER_ITER, infer_rate=INFER_RATE, q_points=QUADRATURE_PTS,
                       net=NET, crf=CRF)
        model.load_weights(weights_dir + '/006.ckpt')
        ...
        tf.keras.backend.set_value(model.optimizer.lr, LEARN_RT)
        tf.keras.backend.set_value(model.optimizer.iterations, 0)

    dataset = get_dataset(shuffle=True, name='cross', scene='train-2012')
    # val_data = get_dataset(shuffle=False, name='real_kitti', scene='validation-201*')
    model.fit(dataset, callbacks=callbacks, epochs=1000, initial_epoch=6)
